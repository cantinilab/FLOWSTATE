...
from .loss import loss_fn, linear_loss
...
class FlowState:
    ...
    def fit(
        ...
        def jitted_update(params, opt_state, batch):
            """A jitted update function. return the new parametrers of the NNn as well as opt state and value of the loss """
        ...
        # The training loop.
        pbar = tqdm(range(1, max_iter + 1)) # ameks a progress bar 
        for it in pbar:
            ...
            if update_train or update_val:
                # Check early stopping. # if val loss has not moved for long enough 
                last_l = self.train_losses[-1] if update_train else self.val_losses[-1]
                early_stop = early_stop.update(last_l) # mdule from flax check if should stop based on loss
    ...
    def transform(
        ...
        def _transform_batch(idx_batch):
            ...
            return self.proximal_step.chained_inference_steps(
                x, a, potential_fun, self.n_steps
            )
